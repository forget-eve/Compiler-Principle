...
```算法
for（∏中的每个子集G）{
把G划分成若干子集，G的两个状态和t在同一子集中，当且仅当对任意输入符号a，s和t的a转换是到∏的同一子集中
在∏new中，用G的划分代替G
}
```
...
> [上下文无关文法](#none)
> [自上而下分析](#from-top-to-bottom)和[自下而上分析]()
> [围绕分析器的自动生成展开]()
...
<span id="from-top-to-bottom"></span>

### 3.3.1 自上而下分析的一般方法
#### 定义
- [x] 自上而下分析的宗旨是，对于任何输入串，试图用一切困难的办法，从文法开始符号(根节点)出发，自上而下，从左到右地为输入串建立分析树。或者说，为输入串寻找最左推导。这种分析过程本质上是一种试探过程，是反复使用不同的产生式谋求匹配输入串的过程。
> 例：若有文法
...
C→cd|c
```
> > 为了自上而下地为输入串w=acb建立分析树，首先建立只有标记为S的单个结点树，输入指针指向w第一个符号a。然后用S的第一个产生式来扩展该树，得到的数如下图所示。

<p align="center">
	<img src="./img/为输入串acb建立分析树.png" alt="为输入串 acb 建立分析树">
        <p align="center">
          <span>为输入串 acb 建立分析树<span>
        </p>
</p>

> > 树中最左边的叶子标记为a，匹配w的符号。于是，推进输入指针到w的第二个符号c，并考虑分析树中的下一个叶子C，它是非终结符。用C的第一个选择来扩展C，得到上图的第二个数。现在第二个输入符号c能匹配，在推进输入指针到b，把它和分析树中的下一个叶子d比较，因为b和d不匹配，回到C,看它是否还有别的选择尚未尝试。
> >
> > 在回到C时，必须让输入指针重新指向第二个符号，和第一次进入C时的位置一致。现在尝试C的第二个选择，得到上图第三个树。叶子c匹配w的第二个符号，叶子b匹配w的第三个符号。这就得到w的分析树，表示分析完全成功。
...
> 不能处理左递归的例子：
> > 如果存在非终结符A，并且有 $A⟹^+Aa$ 这样的左递归，当试图用A去匹配输入串的时候有空使分析过程陷入无限循环(若输入串不是一个句子，则一定陷入无限循环)，因为可能在没有读入任何输入符号时，由要求用下一个A进行新的匹配。
> >
> > 算术表达文法
```文法
E → E + T | T
...
<p align="center">
	<img src="./img/算术表达文法分析树.png" alt="算术表达文法分析树">
        <p align="center">
          <span>算术表达文法分析树<span>
        </p>
</p>
...
### 3.3.2 LL(1)文法
#### 定义
- 为构造不带回溯的自上而下分析算法，首先要消除文法的左递归，并找出避免回溯的充分必要条件。
...
- [x] 表驱动的预测分析器有一个输入缓冲区、一个栈、一张分析表和一个输出流。输入缓冲区包含被分析的串，后面跟一个符号\$，它是输入串的结束标记。栈中存放文法的符号串，栈底符号是\$。初始时，栈中含文法的开始符，它在的上面。分析表是一个二维数组M[A，a]，A是非终结符，a是终结符或\$。
- [x] 现在分析这个分析器的工作过程，。预测分析程序根据当前的栈顶符号X和输入符号a决定分析器的动作，它有四种可能：
	> 1. 如果X=a=\$ ，分析器宣布分析完全成功而停机；

### 3.3.5 构造预测分析表

### 3.3.6 预测分析的错误恢复
- [ ] 先讨论编译器的错误处理
	- [x] 词法错误，如标识符、关键字或算符的拼写错
 	- [x] 语法错误，如算术表达式的括号不配对
	- [x] 语义错误，如算符作用于不相容的运算对象
	- [x] 逻辑错误，如无穷的递归调用和用赋值符号"="代替比较算符"=="
- [ ] 分析器对错误处理的基本目标
	- [x] 清楚而准确地报告错误的出现，并尽量少出现伪错误
	- [x] 迅速地从每个错误中恢复过来，以便诊断后面的错误
	- [x] 它不应该使正确程序的处理速度降低太多 
- [ ] 非递归预测分析在什么场合下发现错误
	- [x] 栈顶是终结符，但和输入符号不匹配
	- [x] 栈顶是非终结符A，输入符号是a，但M[A,a]是空白
